...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    echo "Building Flare..."
    go build -ldflags "-X github.com/ava-labs/avalanchego/version.GitCommit=$git_commit $static_ld_flags" -o "$flare_path" "$FLARE_PATH/main/"*.go
else
    echo "Building Flare with RocksDB enabled..."
    go build -tags rocksdballowed -ldflags "-X github.com/ava-labs/avalanchego/version.GitCommit=$git_commit $static_ld_flags" -o "$flare_path" "$FLARE_PATH/main/"*.go
fi
