...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    echo "Building Flare..."
    go build -modcacherw -ldflags "-X github.com/flare-foundation/flare/version.GitCommit=$git_commit $static_ld_flags" -o "$flare_path" "$FLARE_PATH/main/"*.go
else
    echo "Building Flare with RocksDB enabled..."
    go build -modcacherw -tags rocksdballowed -ldflags "-X github.com/flare-foundation/flare/version.GitCommit=$git_commit $static_ld_flags" -o "$flare_path" "$FLARE_PATH/main/"*.go
fi
