...
func (c *nativeAssetCall) Run(evm *EVM, caller ContractRef, addr common.Address, input []byte, suppliedGas uint64, readOnly bool) (ret []byte, remainingGas uint64, err error) {
	...
	if evm.Context.Time.Cmp(params.NativeAssetCallDeprecationTime) >= 0 {
		return nil, remainingGas, ErrNativeAssetCallDeprecated
	}
	return c.RunNativeAssetCallDeprecated(evm, caller, addr, input, suppliedGas, readOnly)
}

// Run implements StatefulPrecompiledContract
func (c *nativeAssetCall) RunNativeAssetCallDeprecated(evm *EVM, caller ContractRef, addr common.Address, input []byte, suppliedGas uint64, readOnly bool) (ret []byte, remainingGas uint64, err error) {
