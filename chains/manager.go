...
import (
	...
	"github.com/flare-foundation/flare/snow/engine/avalanche/state"
	...
	dbManager "github.com/flare-foundation/flare/database/manager"

	avcon "github.com/flare-foundation/flare/snow/consensus/avalanche"
	aveng "github.com/flare-foundation/flare/snow/engine/avalanche"
	avbootstrap "github.com/flare-foundation/flare/snow/engine/avalanche/bootstrap"
	...
	smbootstrap "github.com/flare-foundation/flare/snow/engine/snowman/bootstrap"
)
...
func (m *manager) buildChain(chainParams ChainParameters, sb Subnet) (*chain, error) {
	...
	// Create the chain
	fmt.Println("Going to call factory New()....")
	vm, err := vmFactory.New(ctx) // todo here we can get the client generated by factory. Next is to use it in windower?
	if err != nil {
	...
	case combinedvm.CombinedVM, []interface {}:
		vmActual := vm.([]interface{})[0].(proposervm.VM)
		//clientFromFactory := vm.([]interface{})[1].(*rpcchainvm.ValidatorsClient)
		chain, err = m.createSnowmanChain(
			ctx,
			chainParams.GenesisData,
			vdrs,
...
func (m *manager) createSnowmanChain(
	...
	fmt.Println("createSnowmanChain called..")
	ctx.Lock.Lock()
	defer ctx.Lock.Unlock()
	...
	if err := vm.Initialize(  //todo inside this windower new is called
		ctx,
		vmDBManager,
		genesisData,
		chainConfig.Upgrade,
