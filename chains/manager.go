...
type manager struct {
	...
	// snowman++ related interface to allow retrieval of P-chain height
...
func (m *manager) createSnowmanChain(
	...
	if ok {
		...
		ctx.ValidatorsUpdater = validation.NewRetrievingUpdater(m.Log, m.Validators, ctx.ValidatorsRetriever)
	}
